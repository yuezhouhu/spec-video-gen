...
```bash
# Basic run (200 prompts from MovieGenVideoBench.txt, outputs to outputs/ablation_random/)
...
`sample_run.py` hardcodes `output_dir="outputs/ablation_random"` and uses `configs/self_forcing_server_14b.yaml`. Videos are saved as `outputs/ablation_random/prompt_NNN.mp4` at 24 fps. The script supports **resume**: it skips already-generated videos.
...
| `RANDOM_ACCEPT_RATE` | `0.7` | Probability of accepting drafter output per block |
...
2. Models loaded once via `load_all(config)` from `release_server.py` (both transformer and drafter loaded)
3. For each prompt: a `GenerationSession` is created, `generate_block()` called `num_blocks` times (default 9)
4. Each block: drafter runs first → random accept/reject (`torch.rand() < RANDOM_ACCEPT_RATE`) → if rejected, transformer re-generates
...
| `release_server.py` | `load_all()`, `GenerateParams`, `GenerationSession`, `load_merge_config()`, routing stats |
