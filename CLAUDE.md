...
Krea Realtime 14B — real-time video generation using a 14B diffusion model distilled from Wan 2.1 via the Self-Forcing technique. This is the **force-reject ablation** repo: block 0 always uses the transformer (target model), blocks 1+ always use the drafter. No ImageReward routing.
...
```bash
# Basic run (200 prompts from MovieGenVideoBench.txt, outputs to outputs/force_reject/)
...
`sample_run.py` hardcodes `output_dir="outputs/force_reject"` and uses `configs/self_forcing_server_14b.yaml`. Videos are saved as `outputs/force_reject/prompt_NNN.mp4` at 24 fps.
...
2. Models loaded once via `load_all(config)` from `release_server.py`
3. For each prompt: a `GenerationSession` is created, `generate_block()` called `num_blocks` times (default 9)
4. Block 0: force reject — full transformer denoising (4–5 steps) → VAE decode
5. Blocks 1+: auto accept — drafter denoising (4–5 steps) → VAE decode
