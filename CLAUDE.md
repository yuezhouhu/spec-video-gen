...
Krea Realtime 14B — real-time video generation using a 14B diffusion model distilled from Wan 2.1 via the Self-Forcing technique. This is the **draft** repo without speculative decoding/ImageReward routing (see `/rscratch/yuezhouhu/realtime-video` for that version).
...
```bash
# Basic run (200 prompts from MovieGenVideoBench.txt, outputs to outputs/draft_only/)
...
`sample_run.py` hardcodes `output_dir="outputs/draft_only"` and uses `configs/self_forcing_server_14b.yaml`. Videos are saved as `outputs/draft_only/prompt_NNN.mp4` at 24 fps.
