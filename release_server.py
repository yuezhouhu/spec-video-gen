...
# Random accept rate for ablation: all blocks are randomly accepted/rejected
RANDOM_ACCEPT_RATE = float(os.getenv("RANDOM_ACCEPT_RATE", "0.7"))
print(f"Routing: RANDOM mode, accept_rate={RANDOM_ACCEPT_RATE}")
...
class GenerationSession:
    ...
    def generate_block_internal(self, models: Models):
        ...
        # --- Routing decision: random accept/reject ---
        ...
        accept = torch.rand((), device=noisy_input.device).item() < RANDOM_ACCEPT_RATE
        log.info(f"Block {self.block_idx}: Random routing, accept={accept} (rate={RANDOM_ACCEPT_RATE})")
        if accept:
