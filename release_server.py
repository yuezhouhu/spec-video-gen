...
# Random routing for ablation: force-reject first block, random accept/reject for the rest
RANDOM_ACCEPT_RATE = float(os.getenv("RANDOM_ACCEPT_RATE", "0.7"))
print(f"Routing: random mode, accept_rate={RANDOM_ACCEPT_RATE}, force_reject_first=True")
...
class GenerationSession:
    ...
    def generate_block_internal(self, models: Models):
        ...
        # --- Routing decision: force-reject first block, random accept/reject for the rest ---
        ...
        if self.block_idx == 0:
            ...
            # Adjusted accept rate for remaining blocks to achieve global target
            # global_rate = (1 * 0 + (num_blocks - 1) * adjusted_rate) / num_blocks
            # adjusted_rate = global_rate * num_blocks / (num_blocks - 1)
            adjusted_rate = min(1.0, (RANDOM_ACCEPT_RATE * self.num_blocks) / max(1, self.num_blocks - 1))
            accept = torch.rand((), device=noisy_input.device).item() < adjusted_rate
            log.info(f"Block {self.block_idx}: Random routing, adjusted_rate={adjusted_rate:.4f}, accept={accept}")
            if accept:
